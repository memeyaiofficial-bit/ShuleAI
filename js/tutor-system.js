...
async function openTutorDashboard() {
  ...
  if (currentUser.type !== "tutor" || !currentUser.email) {
    alert("Please register as a tutor first.");
    return;
  }

  try {
    const response = await fetch(
      `${API_BASE}/tutors/profile/${encodeURIComponent(currentUser.email)}`,
    );
    ...
    if (!data.success || !data.tutor) {
      ...
      localStorage.removeItem("shuleai_current_user");
      localStorage.removeItem("tutorEmail");

      alert("No tutor profile found. Please create your tutor profile first.");

      return;
    }

    window.open(
      `tutor-dashboard.html?email=${encodeURIComponent(currentUser.email)}`,
      "_blank",
    );
  } catch (error) {
    console.error(error);
    alert("Unable to verify tutor profile.");
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const isSignedIn =
    localStorage.getItem("shuleai_signed_in") === "true" ||
    localStorage.getItem("shuleai_unlocked") === "true";
