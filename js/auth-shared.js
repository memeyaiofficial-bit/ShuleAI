...
// Only initialize currentUser if not already set by app.js
...
function playGame(gameName, gameFile) {
  ...
  if (isSignedIn !== "true") {
    // Not signed in - show notification instead of modal
    showQuickNotification("⚠️ Please sign in to play games");
    console.log("User not signed in - game access denied");
    return;
  }
  ...
  if (gameFile) {
    ...
    showQuickNotification("⚠️ Game is coming soon!");
  }
}
...
function handleSpecialCode(
  ...
  const ADMIN_CODE = "SHULEAI-ADMIN-2026";
  ...
  // Set global currentUser object for access checks
  window.currentUser = {
    email: ADMIN_EMAIL,
    daysRemaining: 365,
    accessType: "admin",
  ...
  console.log("📦 Admin localStorage keys:", {
    signed_in: localStorage.getItem("shuleai_signed_in"),
    access_type: localStorage.getItem("shuleai_access_type"),
    daysRemaining: window.currentUser.daysRemaining,
  });
  ...
  // Refresh play buttons with new currentUser value
  if (typeof updatePlayButtons === "function") {
    console.log("🔄 Refreshing play buttons after admin login...");
...
function exploreSubject(subjectName) {
  if (
    !currentUser ||
    !currentUser.daysRemaining ||
    currentUser.daysRemaining <= 0
  ) {
    showQuickNotification("⚠️ Please sign in to access subject resources");
